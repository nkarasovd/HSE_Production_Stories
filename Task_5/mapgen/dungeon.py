        ...
        for room_num, room in enumerate(self.room_list):
            for b in range(room[2]):
                for c in range(room[3]):
                    self.level[room[1] + c][room[0] + b] = TileKind.FREE
                ...
                for width in range(abs(x2 - x3) + 1):
                    for height in range(abs(y2 - y3) + 1):
                        self.level[min(y2, y3) + height][
                            min(x2, x3) + width] = TileKind.FREE

        # paint the walls
        for row in range(1, self.height - 1):
            for col in range(1, self.width - 1):
                if self.level[row][col] == TileKind.FREE:
                    if self.level[row - 1][col - 1] == TileKind.UNKNOWN:
                        self.level[row - 1][col - 1] = TileKind.OCCUPIED

                    if self.level[row - 1][col] == TileKind.UNKNOWN:
                        self.level[row - 1][col] = TileKind.OCCUPIED

                    if self.level[row - 1][col + 1] == TileKind.UNKNOWN:
                        self.level[row - 1][col + 1] = TileKind.OCCUPIED

                    if self.level[row][col - 1] == TileKind.UNKNOWN:
                        self.level[row][col - 1] = TileKind.OCCUPIED
