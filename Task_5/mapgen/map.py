...
from mapgen.agent import Agent, Move, Orientation
...
class Map:
    def __init__(
        ...
        self._explored_area = np.array(explored_area) if explored_area is not None else np.zeros_like(self.tiles,
                                                                                                      dtype=bool)
    ...
    def render(self, agent: Agent):
        ...
        frame = 0.7 * frame + 0.3 * np.tile((1 - self._explored_area * (self.tiles != TileKind.UNKNOWN)) * 200,
